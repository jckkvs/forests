...
- **Composable Architecture**: `ForestBuilder` を通じて、回転、スパース投影、正則化、制約（単調性・線形性）を「トッピング」のように自由に組み合わせて独自のフォレストを構築可能。
...
## 🛠 `ForestBuilder` によるカスマイズ

`ForestBuilder` クラスを使用すると、複雑なフォレスト構成を非常に直感的に記述できます。
