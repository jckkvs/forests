...
==============================
ForestBuilder - トッピング式カスタマイズ

ForestBuilder の引数を変えるだけでアルゴリズムを切り替え可能。
...
for name, kw in configs:
    fb = ForestBuilder(n_estimators=30, random_state=0, task="classification", **kw)
    fb.fit(X_tr, y_tr)
...
for name, kw in reg_configs:
    fb = ForestBuilder(n_estimators=30, random_state=0, task="regression", **kw)
    fb.fit(X2_tr, y2_tr)
